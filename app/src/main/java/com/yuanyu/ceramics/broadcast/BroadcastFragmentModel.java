...
public class BroadcastFragmentModel implements BroadcastFragmentConstract.IBroadcastFragmentModel {
    ...
    @Override
    public Observable<BaseResponse<List<Boolean>>> subscribeLive(String useraccountid, String live_id) {
        String timestamp = Md5Utils.getTimeStamp();
        String randomstr = Md5Utils.getRandomString(10);
        String signature = Md5Utils.getSignature(timestamp,randomstr);
        Map map = new HashMap();
        ...
        map.put("randomstr",randomstr);
        map.put("signature",signature);
        map.put("action","live_subscribe");
        Map data = new HashMap();
        data.put("useraccountid",useraccountid);
        data.put("live_id",live_id);
        map.put("data",data);
        Gson gson=new Gson();
        String str=gson.toJson(map);
        L.e("str is "+str);
        RequestBody body=RequestBody.create(okhttp3.MediaType.parse("application/json; charset=utf-8"),str);
        return httpService.subscribeLive(body);
    }
}
