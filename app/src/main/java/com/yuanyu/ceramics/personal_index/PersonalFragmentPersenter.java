...
public class PersonalFragmentPersenter extends BasePresenter<PersonalFragmentConstract.IPersonalFragmentView> implements PersonalFragmentConstract.IPersonalFragmentPresenter {
    private PersonalFragmentConstract.IPersonalFragmentModel model;

    PersonalFragmentPersenter() {
        model=new PersonalFragmentModel();
    }

    @Override
    public void initList(String useraccountid, String userid,int page, int type) {
        model.initList(useraccountid,userid,page,type)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .compose(new HttpServiceInstance.ErrorTransformer<List<DynamicBean>>())
                .subscribe(new BaseObserver<List<DynamicBean>>() {
                    @Override
                    public void onNext(List<DynamicBean> beans) { if(view!=null){view.initListSuccess(beans);} }

                    @Override
                    public void onError(ExceptionHandler.ResponeThrowable e) { if(view!=null){view.initListFail(e);} }
                });
    }
    @Override
